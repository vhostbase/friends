class Storage
{
	...
	//CREATE TABLE IF NOT EXISTS CHAT_CONTACTS(id, lastSeen, name, number, pic, members)
	createTable(table, columns){
		indexedDB.open = function() {
		  var v = "2.0 beta"; // yes! you can put strings in the version not just numbers
		  var request = indexedDB.open(table, v);
		  
		  request.onupgradeneeded = function(e) {
			var db = request.result;
			var store = db.createObjectStore(table, columns);  
		  };
